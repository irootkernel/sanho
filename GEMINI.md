...
/
├── cmd/
│   └── kkachi-server/    # Server entry point
...
*   **Server State:** The server maintains a single JSON file (default: `kkachi_state.json`) to track projects and workspaces.
...
export PORT=8080
export STATE_FILE_PATH=kkachi_state.json

# Run the server
go run cmd/kkachi-server/main.go
```
