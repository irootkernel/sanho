...
func TestE2E_State(t *testing.T) {
	...
	// Test 6: STASK-2 - Verify /api/state returns identical response to /state
	t.Run("API State endpoint matches State endpoint", func(t *testing.T) {
		// Get /state response
		stateResp, err := client.Get(baseURL + "/state")
		if err != nil {
			t.Fatalf("get /state request failed: %v", err)
		...
		apiStateResp, err := client.Get(baseURL + "/api/state")
		if err != nil {
			t.Fatalf("get /api/state request failed: %v", err)
		}
		if apiStateResp.StatusCode != http.StatusOK {
			t.Fatalf("get /api/state status = %d", apiStateResp.StatusCode)
		}
		var apiStateJSON map[string]interface{}
		json.NewDecoder(apiStateResp.Body).Decode(&apiStateJSON)
		apiStateResp.Body.Close()

		// Compare docs_heads
		stateDocsHeads, ok1 := stateJSON["docs_heads"].(map[string]interface{})
		apiDocsHeads, ok2 := apiStateJSON["docs_heads"].(map[string]interface{})
		if !ok1 || !ok2 {
			t.Fatalf("docs_heads missing in response: /state=%v, /api/state=%v", ok1, ok2)
		}
		if stateDocsHeads[projectName] != apiDocsHeads[projectName] {
			t.Errorf("/state and /api/state docs_heads mismatch for %s: %v vs %v",
				projectName, stateDocsHeads[projectName], apiDocsHeads[projectName])
		}

		// Compare workspaces count
		stateWorkspaces, ok1 := stateJSON["workspaces"].([]interface{})
		apiWorkspaces, ok2 := apiStateJSON["workspaces"].([]interface{})
		if !ok1 || !ok2 {
			t.Fatalf("workspaces missing in response: /state=%v, /api/state=%v", ok1, ok2)
		}
		if len(stateWorkspaces) != len(apiWorkspaces) {
			t.Errorf("/state and /api/state workspaces count mismatch: %d vs %d",
				len(stateWorkspaces), len(apiWorkspaces))
		}
	})
}
