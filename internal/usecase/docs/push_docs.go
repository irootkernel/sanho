...
func (u *pushDocsUseCase) Execute(ctx context.Context, cmd PushDocsCommand) (domain.DocsPushResult, error) {
	...
	case domain.DocsPushStatusUpdated:
		if result.NewHead == nil {
			...
			return domain.DocsPushResult{}, domain.ErrInconsistentPush
		}
	...
	if result.Status == domain.DocsPushStatusUpdated {
		if err := u.docsWriteRepo.Push(ctx, ws.Project); err != nil {
			// Best-effort rollback of state to previous hash to stay in sync with remote
			if rollbackErr := u.workspaceRepo.UpdateDocsHash(ctx, workspace.WorkspaceID(cmd.WorkspaceID), previousHash, cmd.ActorEmail); rollbackErr != nil {
