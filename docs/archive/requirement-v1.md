...
- 주의사항
  ...
  - `force=true` 옵션은 **위험도가 높은 동작**이므로, kkachi CLI (`kkachi project delete`) 에서는
    - 기본적으로 `force=false` 로 호출하고, 서버에서 `project_has_workspaces`(409) 를 받으면  
      "등록된 workspace 가 남아 있어 project 를 삭제할 수 없습니다. 먼저 해당 workspace 들을 정리한 뒤 다시 시도해 주세요." 와 같은 안내를 출력하고 exit code 1 로 처리한다.
    - 사용자가 명시적으로 `--force` 플래그를 지정한 경우에만 `force=true` 로 호출해, 등록된 workspace 가 남아 있어도 삭제를 강행할 수 있다.
...
1. 현재 디렉토리 유효성 검사
   ...
   - `.git` 디렉토리가 있는지 확인
   - 없으면 에러 출력 후 종료
   - 이미 `.kkachi.json` 이 존재하는 경우
     - 기본 동작(`--force` 미사용 시): **init 실패(exit 1)** 로 간주한다.
       - "현재 디렉토리는 이미 kkachi workspace 입니다. 설정을 초기화하려면 `kkachi clean` 으로 정리하거나, `kkachi init --force` 를 사용해 재초기화해 주세요." 와 같은 안내를 출력한다.
     - `kkachi init --force` 사용 시:
       - 기존 `.kkachi.json`, `docs_hash_file`, `.kkachi_pending_fix` 상태를 모두 무시하고 **fresh init** 으로 간주한다.
       - 이후 단계에서 서버 snapshot 을 기준으로 docs 디렉토리를 다시 생성·덮어쓰며, `.kkachi_docs_hash` 도 서버 HEAD 기준으로 재설정한다.
   - docs 디렉토리 존재 여부 확인 (`--force` 미사용 시)
...
- CLI 서브커맨드 추가
  - `kkachi status --verbose` 와 같은 상세 모드
    - v1 기준 기본 `kkachi status` 출력은 사람이 읽기 좋은 요약 위주이며,
      향후 JSON/머신 친화적인 상세 모드나 추가 필드(`blocked`, `pending_fix` 등)를 제공할 수 있다.
