...
### 3.1 목표
- Web에서 PTY 세션을 띄워 **쉘/AI CLI 실행**이 가능하게 한다.
...
- (선택, v3 내 UX 확장) 복수 콘솔(세션) 지원:
  ...
  - 드래그&드롭으로 콘솔 순서 변경
  - 주: 세션 복구/재연결은 여전히 v3 비범위(WS 끊김 시 정리 정책 유지)
...
### 3.7 구현 문서(참고)
- v3 상세 요구사항/스펙: `requirement.md`
- v3 서버 작업 계획: `server-roadmap.md`
- v3 클라이언트 작업 계획: `client-roadmap.md`
